...
answers %<>% gather(key = unit, value = time_diff, ... = time_diff_sec:time_diff_days) %>% 
  mutate(unit = str_replace(pattern = "time_diff", replacement = ""))
...
answers.filt %>% 
  ...
  geom_label(aes(label = n), vjust = "inward", nudge_y = 0.25)
```
...
TODO: build an understanding of the features that predict the time it takes for a question to get a response.
